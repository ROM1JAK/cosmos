...
function switchView(view) {
    ...
    if(view === 'feed') {
        ...
        loadFeed();
        loadStories(); // Load stories on feed visit
    }
    if(view === 'music') {
        loadMusicFeed();
    }
...
function updateClock() {
    const now = new Date();
    document.getElementById('realtime-clock').textContent = now.toLocaleTimeString('fr-FR', { timeZone: 'Europe/Paris', hour: '2-digit', minute: '2-digit' });
}
setInterval(updateClock, 1000); updateClock();

async function toggleRecording(source) { 
    const btn = document.getElementById(`btn-record-${source}`);
    if (!btn) return; 
    if (!isRecording) {
        try {
...
document.getElementById('txtInput').addEventListener('input', function(e) {
    const input = e.target; const cursor = input.selectionStart;
    const textBefore = input.value.substring(0, cursor); const lastWord = textBefore.split(/\s/).pop();
    const suggestionsBox = document.getElementById('mention-suggestions');
    if (lastWord.startsWith('@')) {
        const query = lastWord.substring(1).toLowerCase(); const matches = allOnlineUsers.filter(u => u.toLowerCase().startsWith(query));
        if (matches.length > 0) {
            suggestionsBox.innerHTML = '';
            matches.forEach(match => {
                const div = document.createElement('div'); div.className = 'mention-item'; div.textContent = match;
                div.onclick = () => { input.value = textBefore.substring(0, textBefore.length - lastWord.length) + '@' + match + ' ' + input.value.substring(cursor); input.focus(); suggestionsBox.classList.add('hidden'); };
...
socket.on('login_success', (data) => {
    localStorage.setItem('rp_username', data.username); localStorage.setItem('rp_code', data.userId);
...
function joinRoom(roomId) {
    ...
    localStorage.setItem('saved_room_id', roomId); currentDmTarget = null; 
    socket.emit('join_room', currentRoomId); if (unreadRooms.has(currentRoomId)) unreadRooms.delete(currentRoomId);
    const room = allRooms.find(r => r._id === roomId);
    document.getElementById('currentRoomName').textContent = room ? room.name : 'Salon Global'; document.getElementById('currentRoomName').style.color = "var(--text-primary)";
...
function updateRoomListUI() {
    ...
    allRooms.forEach(room => {
        ...
        const isUnread = unreadRooms.has(String(room._id)) ? 'unread' : ''; const isActive = (String(currentRoomId) === String(room._id) && !currentDmTarget) ? 'active' : '';
...
function openDm(target) {
    ...
    if (!dmContacts.includes(target)) dmContacts.push(target); if (unreadDms.has(target)) unreadDms.delete(target);
...
socket.on('receive_dm', (msg) => {
    ...
    if (currentDmTarget === other) { displayMessage(msg, true); scrollToBottom(); } else { unreadDms.add(other); updateDmListUI(); }
...
async function createCharacter() {
    ...
    const name = document.getElementById('newCharName').value.trim(); const role = document.getElementById('newCharRole').value.trim();
...
async function submitEditCharacter() {
    const file = document.getElementById('editCharFile').files[0]; let newAvatar = document.getElementById('editCharBase64').value; 
    if (file) { const url = await uploadToCloudinary(file); if (url) newAvatar = url; }
    socket.emit('edit_char', { 
        charId: document.getElementById('editCharId').value, originalName: document.getElementById('editCharOriginalName').value, 
        newName: document.getElementById('editCharName').value.trim(), newRole: document.getElementById('editCharRole').value.trim(), 
        newAvatar, newColor: document.getElementById('editCharColor').value, newDescription: document.getElementById('editCharDesc').value.trim(), 
...
socket.on('my_chars_data', (chars) => { 
    ...
    if (saved && myCharacters.find(c => c._id === saved)) selectCharacter(saved); else if (IS_ADMIN && saved === 'narrateur') selectCharacter('narrateur');
});
...
function toggleCharBar() {
    ...
    if (bar.classList.contains('hidden-bar')) { icon.classList.remove('fa-chevron-down'); icon.classList.add('fa-chevron-up'); } else { icon.classList.remove('fa-chevron-up'); icon.classList.add('fa-chevron-down'); }
}

function updateUI() {
    const list = document.getElementById('myCharList'); const bar = document.getElementById('char-bar-horizontal');
    list.innerHTML = ""; bar.innerHTML = ""; const widget = document.getElementById('feed-identity-widget'); widget.innerHTML = "";
    ...
    let activeHTML = activeChar ? `<img src="${activeChar.avatar}" class="feed-avatar-choice active-feed-char"> <span style="font-weight:bold; color:white; font-size:0.9rem;">${activeChar.name}</span>` : `<span style="color:#aaa;">Sélectionner</span>`;
    widget.innerHTML = `<div class="identity-active" onclick="toggleIdentityDropdown()">${activeHTML}<i class="fa-solid fa-chevron-down" style="color:#aaa; font-size:0.8rem; margin-left:auto;"></i></div><div id="identity-dropdown-list" class="identity-dropdown hidden"></div>`;
    const dropdown = document.getElementById('identity-dropdown-list');
    myCharacters.forEach(char => {
        const item = document.createElement('div'); item.className = 'identity-option'; item.innerHTML = `<img src="${char.avatar}" class="feed-avatar-choice"> <span style="color:#ddd; font-size:0.9rem;">${char.name}</span>`;
        item.onclick = () => { currentFeedCharId = char._id; updateUI(); }; dropdown.appendChild(item);
    });
}
function toggleIdentityDropdown() { document.getElementById('identity-dropdown-list').classList.toggle('hidden'); }

// --- PROFILE ---
function openProfile(name) { document.getElementById('profile-overlay').classList.remove('hidden'); document.getElementById('profile-slide-panel').classList.add('open'); socket.emit('get_char_profile', name); }
function closeProfileModal() { document.getElementById('profile-slide-panel').classList.remove('open'); document.getElementById('profile-overlay').classList.add('hidden'); }
socket.on('char_profile_data', (char) => {
    document.getElementById('profileName').textContent = char.name; document.getElementById('profileRole').textContent = char.role; document.getElementById('profileAvatar').src = char.avatar;
    document.getElementById('profileDesc').textContent = char.description || "Aucune description."; document.getElementById('profileOwner').textContent = `Joué par : ${char.ownerUsername || "Inconnu"}`;
    document.getElementById('profilePostCount').textContent = char.postCount || 0;
    document.getElementById('profileFollowersCount').textContent = `${char.followers ? char.followers.length : 0}`;
    ...
    if(currentFeedCharId === char._id) { btnSub.style.display = 'none'; } else {
        btnSub.style.display = 'block'; const isSubbed = char.followers && currentFeedCharId && char.followers.includes(currentFeedCharId);
        updateSubButton(btnSub, isSubbed);
        btnSub.onclick = function() { if(!currentFeedCharId) return alert("Sélectionnez un perso dans le Feed !"); socket.emit('follow_character', { followerCharId: currentFeedCharId, targetCharId: char._id }); };
    }
});
socket.on('char_profile_updated', (char) => { 
    if(document.getElementById('profile-slide-panel').classList.contains('open') && document.getElementById('profileName').textContent === char.name) {
        const isSubbed = char.followers && currentFeedCharId && char.followers.includes(currentFeedCharId); updateSubButton(document.getElementById('btn-sub-profile'), isSubbed);
...
socket.on('followers_list_data', (followers) => {
    ...
    followers.forEach(f => { listDiv.innerHTML += `<div style="display:flex; align-items:center; padding:8px; border-bottom:1px solid #333;"><img src="${f.avatar}" style="width:30px; height:30px; border-radius:50%; margin-right:10px;"><div><div style="font-weight:bold;">${f.name}</div><div style="font-size:0.8em; color:#aaa;">${f.role}</div></div></div>`; });
    document.getElementById('followers-modal').classList.remove('hidden');
});

// --- ACTIONS MSG ---
function setContext(type, data) {
    currentContext = { type, data }; const bar = document.getElementById('context-bar'); bar.className = 'visible';
...
async function sendMessage() {
    const txt = document.getElementById('txtInput'); const content = txt.value.trim(); let finalMediaUrl = null, finalMediaType = null;
    if (pendingAttachment) {
        document.getElementById('chat-staging').innerHTML = 'Envoi...'; let rType = (pendingAttachment.type === 'audio') ? 'video' : undefined;
        finalMediaUrl = await uploadToCloudinary(pendingAttachment.file, rType); finalMediaType = pendingAttachment.type; clearStaging(); if (!finalMediaUrl) return alert("Echec envoi média.");
    }
    if (!content && !finalMediaUrl) return;
    if (currentDmTarget) { socket.emit('send_dm', { sender: USERNAME, target: currentDmTarget, content: content || finalMediaUrl, type: finalMediaType || "text", date: new Date().toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'}) }); txt.value = ''; cancelContext(); return; }
...
socket.on('history_data', (msgs) => { 
    ...
    if(firstUnreadMap[currentRoomId]) delete firstUnreadMap[currentRoomId]; scrollToBottom(); 
});
...
function displayMessage(msg, isDm = false) {
    ...
    if (isGroup) { div.classList.add('msg-group-followup'); div.dataset.timeShort = msg.date; } else { lastMessageData = { author: senderName, time: msgTime }; }
    let actionsHTML = "";
    if (!isDm) {
    ...
    else if (msg.type === "audio") { contentHTML = `<div id="audio-placeholder-${msg._id}"></div>`; } else contentHTML = `<div class="text-body" id="content-${msg._id}">${formatText(msg.content)}</div>`;
    ...
    let replyHTML = ""; if (msg.replyTo && msg.replyTo.author) { replyHTML = `<div class="reply-context-line"><div class="reply-spine"></div><span style="font-weight:600; cursor:pointer;">@${msg.replyTo.author}</span> <span style="font-style:italic; opacity:0.8;">${msg.replyTo.content}</span></div>`; }
    let innerHTML = ""; if(replyHTML) innerHTML += replyHTML;
    innerHTML += `<div style="display:flex; width:100%;">`;
    innerHTML += `<div class="msg-col-avatar">`; if(!isGroup) { innerHTML += `<img src="${senderAvatar}" class="avatar-img" ${avatarClick}>`; } innerHTML += `</div><div class="msg-col-content">`;
    ...
    innerHTML += contentHTML + editedTag; innerHTML += `</div>${actionsHTML}</div>`;
    div.innerHTML = innerHTML; document.getElementById('messages').appendChild(div);
...
async function previewPostFile() {
    ...
    if(file) {
        ...
        const url = await uploadToCloudinary(file); if(url) { document.getElementById('postMediaUrl').value = url; document.getElementById('postFileStatus').textContent = "Prêt !"; }
    }
}
function submitPost() {
    const content = document.getElementById('postContent').value.trim(); const mediaUrl = document.getElementById('postMediaUrl').value.trim();
    if(!content && !mediaUrl) return alert("Contenu vide.");
    if(!currentFeedCharId) return alert("Sélectionnez un perso pour le Feed.");
    const char = myCharacters.find(c => c._id === currentFeedCharId); if(!char) return alert("Perso invalide.");
    let mediaType = null;
    if(mediaUrl) {
        if(getYoutubeId(mediaUrl) || mediaUrl.match(/\.(mp4|webm|ogg)$/i) || mediaUrl.includes('/video/upload')) mediaType = 'video';
        else if (mediaUrl.includes('.webm') || mediaUrl.includes('/raw/upload') && !mediaUrl.includes('image')) mediaType = 'audio'; else mediaType = 'image';
        if(mediaUrl.endsWith('.webm') && !mediaType) mediaType = 'video'; 
    }
    socket.emit('create_post', { authorCharId: char._id, authorName: char.name, authorAvatar: char.avatar, authorRole: char.role, content, mediaUrl, mediaType, date: new Date().toLocaleDateString(), ownerId: PLAYER_ID });
...
function submitRepost(originalPostId) {
    if(!currentFeedCharId) return alert("Sélectionnez un perso pour le Feed.");
    const char = myCharacters.find(c => c._id === currentFeedCharId);
    if(confirm("Reposter cette publication ?")) {
...
function createPostElement(post) {
    ...
    let isRepost = !!post.repostOf;
    let displayPost = isRepost ? post.repostOf : post; // If deleted original, displayPost might be null? 
    if(isRepost && !displayPost) { div.innerHTML = "<div style='color:#666; font-style:italic;'>Publication originale supprimée.</div>"; return div; }

    const isLiked = displayPost.likes.includes(currentFeedCharId); 
    ...
    if(displayPost.mediaUrl) {
        if(displayPost.mediaType === 'video' || displayPost.mediaUrl.includes('/video/upload')) {
             const ytId = getYoutubeId(displayPost.mediaUrl);
             if(ytId) mediaHTML = `<iframe class="post-media" src="https://www.youtube.com/embed/${ytId}" frameborder="0" allowfullscreen></iframe>`;
             else mediaHTML = `<video class="post-media" controls src="${displayPost.mediaUrl}"></video>`;
        } else if (displayPost.mediaType === 'audio') { mediaHTML = `<audio controls src="${displayPost.mediaUrl}" style="width:100%; margin-top:10px;"></audio>`; } 
        else { mediaHTML = `<img src="${displayPost.mediaUrl}" class="post-media">`; }
    }

    let repostHeader = "";
    if(isRepost) {
        repostHeader = `<div class="repost-header"><i class="fa-solid fa-retweet"></i> ${post.authorName} a reposté</div>`;
    }

    div.innerHTML = `${delBtn} ${reportBtn}
        ${repostHeader}
        <div class="${isRepost ? 'repost-content' : ''}">
            <div class="post-header" onclick="event.stopPropagation(); openProfile('${displayPost.authorName.replace(/'/g, "\\'")}')">
                <img src="${displayPost.authorAvatar}" class="post-avatar">
                <div class="post-meta">
                    <div class="post-author">${displayPost.authorName}</div>
                    <div class="post-role">${displayPost.authorRole}</div>
                </div>
                <span class="post-date">${displayPost.date}</span>
            </div>
            <div class="post-content" onclick="openPostDetail('${displayPost._id}')">${formatText(displayPost.content)}</div>
            ${mediaHTML}
        </div>
        <div class="post-actions">
            <button class="action-item ${isLiked?'liked':''} liked-btn" onclick="event.stopPropagation(); toggleLike('${displayPost._id}')"><i class="fa-solid fa-heart"></i> ${displayPost.likes.length}</button>
            <button class="action-item" onclick="event.stopPropagation(); openPostDetail('${displayPost._id}')"><i class="fa-solid fa-comment"></i> ${displayPost.comments.length}</button>
            <button class="action-item" onclick="event.stopPropagation(); submitRepost('${displayPost._id}')"><i class="fa-solid fa-retweet"></i></button>
        </div>
        <div class="comments-list hidden">${generateCommentsHTML(displayPost.comments, displayPost._id)}</div>`;
    return div;
}

// --- STORIES LOGIC ---
async function previewAndUploadStory(input) {
    if(!input.files || !input.files[0]) return;
    if(!currentFeedCharId) return alert("Sélectionnez un personnage (à droite).");
    const char = myCharacters.find(c => c._id === currentFeedCharId);
    
    if(confirm("Publier en Story ?")) {
        const file = input.files[0];
        const url = await uploadToCloudinary(file);
        if(!url) return alert("Echec upload.");
        
        const type = file.type.startsWith('image') ? 'image' : 'video';
        socket.emit('upload_story', {
...
function loadStories() { socket.emit('get_stories'); }

socket.on('stories_data', (groups) => {
    groupedStories = groups;
    renderStoriesBar();
});
socket.on('story_uploaded', (s) => {
    // Add to local data or reload
    loadStories(); 
});
...
function renderStoriesBar() {
    ...
    container.innerHTML = `
        <div class="story-circle" onclick="document.getElementById('story-upload-input').click()">
            <div class="story-ring story-add-btn">+</div>
            <span class="story-name">Moi</span>
        </div>
    `;
    ...
    groupedStories.forEach((group, index) => {
        // Check if seen all
        let allSeen = true;
        for(let s of group.stories) {
            if(!localStorage.getItem(`seen_story_${s._id}`)) { allSeen = false; break; }
        }
        
        container.innerHTML += `
        <div class="story-circle" onclick="openStoryViewer(${index})">
            <div class="story-ring ${allSeen ? 'seen' : ''}">
                <img src="${group.authorAvatar}" class="story-avatar">
            </div>
            <span class="story-name">${group.authorName}</span>
        </div>`;
    });
}
...
function openStoryViewer(groupIndex) {
    ...
    currentStoryIndex = 0;
    // Find first unseen story in this group
    const group = groupedStories[groupIndex];
    for(let i=0; i<group.stories.length; i++) {
        if(!localStorage.getItem(`seen_story_${group.stories[i]._id}`)) {
            currentStoryIndex = i; break;
        }
    }
...
function showStory() {
    ...
    document.getElementById('story-viewer-name').textContent = group.authorName;
    document.getElementById('story-viewer-avatar').src = group.authorAvatar;
    // Calc time ago simplified
    document.getElementById('story-viewer-time').textContent = "À l'instant"; 
    
    const container = document.getElementById('story-media-container');
    container.innerHTML = "";
    
    if(story.mediaType === 'image') {
        container.innerHTML = `<img src="${story.mediaUrl}" class="story-media">`;
        resetStoryTimer(5000);
    } else {
        ...
        resetStoryTimer(null); // Video controls time
    }
    
    // Mark as seen
    localStorage.setItem(`seen_story_${story._id}`, 'true');
    renderStoriesBar(); // Update ring color
}

function resetStoryTimer(ms) {
    ...
    if(ms) {
        setTimeout(() => {
            document.getElementById('story-progress-bar').style.transition = `width ${ms}ms linear`;
...
function prevStory() {
    if(currentStoryIndex > 0) {
        ...
        if(currentStoryGroupIndex > 0) {
            currentStoryGroupIndex--;
            currentStoryIndex = groupedStories[currentStoryGroupIndex].stories.length - 1;
            showStory();
        }
    }
}
...
function reportPost(id) { if(confirm("Signaler ce post à la modération ?")) { socket.emit('report_post', id); alert("Signalement envoyé."); } }
let currentDetailPostId = null;
function openPostDetail(id) {
    const post = allPosts.find(p => p._id === id); if(!post) return;
    const isRepost = !!post.repostOf;
    const displayPost = isRepost ? post.repostOf : post;
    if(!displayPost) return;

    currentDetailPostId = id; // Keep track of the wrapper ID (repost or original)
    const tempDiv = document.createElement('div'); tempDiv.appendChild(createPostElement(post));
    const postContentClone = tempDiv.querySelector('.post-card').cloneNode(true);
    postContentClone.onclick = null; postContentClone.style.border = "none"; postContentClone.classList.remove('highlight-new');
    const oldComments = postContentClone.querySelector('.comments-list'); if(oldComments) oldComments.remove();
    document.getElementById('post-detail-content').innerHTML = ""; document.getElementById('post-detail-content').appendChild(postContentClone);
    document.getElementById('post-detail-comments-list').innerHTML = generateCommentsHTML(displayPost.comments, displayPost._id); // Comments belong to ORIGINAL post
    ...
    document.getElementById('btn-detail-comment').onclick = async () => {
        const txt = document.getElementById('post-detail-comment-input').value.trim(); let mediaUrl = null, mediaType = null;
        if(pendingCommentAttachment && pendingCommentAttachment.files[0]) { let rType = (pendingCommentAttachment.type === 'audio') ? 'video' : undefined; mediaUrl = await uploadToCloudinary(pendingCommentAttachment.files[0], rType); mediaType = pendingCommentAttachment.type; }
        if(!txt && !mediaUrl) return;
        if(!currentFeedCharId) return alert("Sélectionnez un perso (Feed)."); const char = myCharacters.find(c => c._id === currentFeedCharId);
        socket.emit('post_comment', { postId: displayPost._id, comment: { authorCharId: char._id, authorName: char.name, authorAvatar: char.avatar, content: txt, mediaUrl, mediaType, date: new Date().toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'}), ownerId: PLAYER_ID } });
...
function stageCommentMedia(input, forcedType) {
    const file = input.files[0]; if(!file) return; let type = forcedType || (file.type.startsWith('image') ? 'image' : 'video');
    pendingCommentAttachment = { files: input.files, type };
    document.getElementById('comment-staging').classList.remove('hidden'); document.getElementById('comment-staging').innerHTML = `<span class="staging-info">${type} prêt</span> <button class="btn-clear-stage" onclick="clearCommentStaging()">X</button>`;
}
...
socket.on('feed_data', (posts) => { allPosts = posts; const c = document.getElementById('feed-stream'); c.innerHTML = ""; posts.forEach(p => c.appendChild(createPostElement(p))); });
socket.on('new_post', (post) => { allPosts.unshift(post); if(currentView !== 'feed') document.getElementById('btn-view-feed').classList.add('nav-notify'); document.getElementById('feed-stream').prepend(createPostElement(post)); });
socket.on('post_updated', (post) => {
    const idx = allPosts.findIndex(p => p._id === post._id); if(idx !== -1) allPosts[idx] = post;
    const el = document.getElementById(`post-${post._id}`); if(el) el.replaceWith(createPostElement(post));
    if(currentDetailPostId && (currentDetailPostId === post._id || (allPosts.find(p=>p._id===currentDetailPostId)?.repostOf?._id === post._id))) {
        // If we are viewing detail of this post OR a repost of this post
        openPostDetail(currentDetailPostId);
    }
});
socket.on('post_deleted', (id) => { allPosts = allPosts.filter(p => p._id !== id); const el = document.getElementById(`post-${id}`); if(el) el.remove(); if(currentDetailPostId === id) closePostDetail(); });
socket.on('reload_posts', () => loadFeed());
function generateCommentsHTML(comments, postId) {
    let html = ""; comments.forEach(c => {
        const delBtn = IS_ADMIN ? `<span style="color:#da373c; cursor:pointer; margin-left:10px;" onclick="deleteComment('${postId}', '${c.id}')">X</span>` : "";
        let mediaHtml = ""; if(c.mediaUrl) { if(c.mediaType === 'image') mediaHtml = `<img src="${c.mediaUrl}" style="max-width:200px; border-radius:4px; margin-top:5px;">`; if(c.mediaType === 'video') mediaHtml = `<video src="${c.mediaUrl}" controls style="max-width:200px; border-radius:4px; margin-top:5px;"></video>`; if(c.mediaType === 'audio') mediaHtml = `<audio src="${c.mediaUrl}" controls style="max-width:200px; margin-top:5px;"></audio>`; }
        const likesCount = c.likes ? c.likes.length : 0; const isLiked = c.likes && currentFeedCharId && c.likes.includes(currentFeedCharId);
        html += `<div class="comment-item"><div class="comment-bubble"><div class="comment-meta"><img src="${c.authorAvatar}" style="width:20px;height:20px;border-radius:50%;margin-right:5px;"><b>${c.authorName}</b> ${c.date}</div><div style="margin-left:25px;" class="text-body">${c.content} ${mediaHtml} ${delBtn}</div><div class="comment-actions-bar"><button class="btn-comment-action ${isLiked?'liked':''}" onclick="toggleCommentLike('${postId}', '${c.id}')"><i class="fa-solid fa-heart"></i> ${likesCount}</button></div></div></div>`;
    }); return html;
}
let notifications = [];
...
function updateNotificationBadge() { const c = notifications.filter(n => !n.isRead).length; const b = document.getElementById('notif-badge'); if(c > 0) { b.textContent = c; b.classList.remove('hidden'); } else b.classList.add('hidden'); }
function openNotifications() { document.getElementById('notifications-modal').classList.remove('hidden'); const list = document.getElementById('notif-list'); list.innerHTML = ""; if(notifications.length === 0) list.innerHTML = "<div style='text-align:center; padding:20px; color:#777'>Rien.</div>"; notifications.forEach(n => { list.innerHTML += `<div class="notif-item ${!n.isRead?'unread':''}"><div class="notif-icon"><i class="fa-solid fa-bell"></i></div><div class="notif-content"><strong>${n.fromName}</strong> ${n.content}</div></div>`; }); socket.emit('mark_notifications_read', PLAYER_ID); notifications.forEach(n=>n.isRead=true); updateNotificationBadge(); }
function closeNotifications() { document.getElementById('notifications-modal').classList.add('hidden'); }
async function submitTrack() {
    const title = document.getElementById('trackTitle').value.trim(); const audioFile = document.getElementById('trackAudioFile').files[0]; const coverFile = document.getElementById('trackCoverFile').files[0];
    if(!title || !audioFile || !coverFile) return alert("Tout est requis (Titre, Audio, Cover)."); if(!currentFeedCharId) return alert("Sélectionnez un perso artiste (à droite).");
    const char = myCharacters.find(c => c._id === currentFeedCharId); const statusDiv = document.getElementById('track-upload-status'); statusDiv.textContent = "Upload Audio...";
    const audioUrl = await uploadToCloudinary(audioFile, 'video'); if(!audioUrl) return statusDiv.textContent = "Erreur Audio.";
    statusDiv.textContent = "Upload Cover..."; const coverUrl = await uploadToCloudinary(coverFile, 'image'); if(!coverUrl) return statusDiv.textContent = "Erreur Cover.";
    statusDiv.textContent = "Publication...";
    socket.emit('upload_track', { title, audioUrl, coverUrl, artistId: char._id, artistName: char.name, ownerId: PLAYER_ID });
    document.getElementById('music-upload-modal').classList.add('hidden'); document.getElementById('trackTitle').value = ""; document.getElementById('trackAudioFile').value = ""; document.getElementById('trackCoverFile').value = ""; statusDiv.textContent = "";
}
function previewTrackAudio() { const f = document.getElementById('trackAudioFile').files[0]; if(f) document.getElementById('audio-preview-name').textContent = f.name; }
function previewTrackCover() { const f = document.getElementById('trackCoverFile').files[0]; if(f) { const img = document.getElementById('cover-preview-img'); img.src = URL.createObjectURL(f); img.classList.remove('hidden'); } }
function loadMusicFeed() { socket.emit('get_music_feed'); }
socket.on('music_feed_data', (tracks) => { musicPlaylist = tracks; renderMusicGrid(tracks); });
socket.on('track_uploaded', (track) => { musicPlaylist.unshift(track); renderMusicGrid(musicPlaylist); });
socket.on('track_updated', (track) => { const idx = musicPlaylist.findIndex(t => t._id === track._id); if(idx !== -1) musicPlaylist[idx] = track; renderMusicGrid(musicPlaylist); if(isPlaying && musicPlaylist[currentTrackIndex]._id === track._id) updatePlayerUI(track); });
socket.on('track_deleted', (trackId) => { musicPlaylist = musicPlaylist.filter(t => t._id !== trackId); renderMusicGrid(musicPlaylist); if(isPlaying && musicPlaylist[currentTrackIndex] && musicPlaylist[currentTrackIndex]._id === trackId) { globalAudio.pause(); isPlaying = false; document.getElementById('persistent-player-bar').classList.add('hidden'); } });
function renderMusicGrid(tracks) {
    const grid = document.getElementById('music-grid'); grid.innerHTML = "";
    tracks.forEach((t, index) => {
        const isLiked = t.likes.includes(currentFeedCharId); const canDelete = (t.ownerId === PLAYER_ID || IS_ADMIN);
        ...
        grid.innerHTML += `<div class="music-card" onclick="playTrack(${index})">${delBtn}<div class="cover-art-container"><img src="${t.coverUrl}" class="cover-art"><div class="play-overlay"><div class="overlay-icon"><i class="fa-solid fa-play"></i></div></div></div><div class="track-info"><div class="track-title">${t.title}</div><div class="track-artist">${t.artistName}</div><div class="track-stats"><span><i class="fa-solid fa-play"></i> ${t.plays}</span><span style="${isLiked?'color:#da373c':''}"><i class="fa-solid fa-heart"></i> ${t.likes.length}</span></div></div></div>`;
    });
}
function deleteTrack(id) { if(confirm("Supprimer ce son ?")) socket.emit('delete_track', id); }
function playTrack(index) {
    if(index < 0 || index >= musicPlaylist.length) return; currentTrackIndex = index; const track = musicPlaylist[index];
    globalAudio.src = track.audioUrl; globalAudio.play(); isPlaying = true;
    document.getElementById('persistent-player-bar').classList.remove('hidden'); updatePlayerUI(track); socket.emit('listen_track', track._id);
}
function updatePlayerUI(track) {
    document.getElementById('player-cover').src = track.coverUrl; document.getElementById('player-title').textContent = track.title; document.getElementById('player-artist').textContent = track.artistName;
    document.getElementById('player-play-btn').innerHTML = isPlaying ? '<i class="fa-solid fa-pause"></i>' : '<i class="fa-solid fa-play"></i>';
    const isLiked = track.likes.includes(currentFeedCharId); document.getElementById('player-like-btn').innerHTML = isLiked ? '<i class="fa-solid fa-heart" style="color:#da373c;"></i>' : '<i class="fa-regular fa-heart"></i>';
    const visualizer = document.getElementById('player-visualizer'); if(isPlaying) visualizer.classList.remove('hidden'); else visualizer.classList.add('hidden');
}
function togglePlay() { if(globalAudio.paused) { globalAudio.play(); isPlaying = true; } else { globalAudio.pause(); isPlaying = false; } if(currentTrackIndex !== -1) updatePlayerUI(musicPlaylist[currentTrackIndex]); }
function prevTrack() { playTrack(currentTrackIndex - 1); } function nextTrack() { playTrack(currentTrackIndex + 1); }
function togglePlayerLike() { if(currentTrackIndex === -1) return; if(!currentFeedCharId) return alert("Sélectionnez un perso artiste."); const track = musicPlaylist[currentTrackIndex]; socket.emit('like_track', { trackId: track._id, charId: currentFeedCharId }); }
globalAudio.addEventListener('timeupdate', () => { const percent = (globalAudio.currentTime / globalAudio.duration) * 100; document.getElementById('player-progress-bar').style.width = percent + '%'; document.getElementById('player-current-time').textContent = formatTime(globalAudio.currentTime); });
globalAudio.addEventListener('loadedmetadata', () => { document.getElementById('player-duration').textContent = formatTime(globalAudio.duration); });
...
function seekTrack(e) { const width = e.target.clientWidth; const clickX = e.offsetX; const duration = globalAudio.duration; globalAudio.currentTime = (clickX / width) * duration; }

// Toggle Player Collapse
function togglePlayerBar() {
    document.getElementById('persistent-player-bar').classList.toggle('player-collapsed');
}
