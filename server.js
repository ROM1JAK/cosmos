...
const RoomSchema = new mongoose.Schema({
    name: { type: String, required: true }, creatorId: String
});
const Room = mongoose.model('Room', RoomSchema);

// --- GESTION UTILISATEURS EN LIGNE ---
let onlineUsers = {}; // Stocke { socketId: username }

function broadcastUserList() {
    // On récupère les pseudos uniques
...
io.on('connection', async (socket) => {
  ...
  socket.on('edit_char', async (data) => {
      await Character.updateOne({ name: data.originalName, ownerId: data.ownerId }, { ...data });
