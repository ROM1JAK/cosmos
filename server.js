...
const PostSchema = new mongoose.Schema({
    ...
    likes: [String], 
    repostOf: { type: mongoose.Schema.Types.ObjectId, ref: 'Post' }, // REPOST FEATURE - NEW
    comments: [{
...
io.on('connection', async (socket) => {
  ...
  socket.on('follow_character', async ({ followerCharId, targetCharId }) => {
      ...
      if(!target || !follower || target.ownerId === follower.ownerId) return; 
